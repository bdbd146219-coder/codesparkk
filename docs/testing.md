...
**QA-only route production gate (C4V).** `src/app/__tests__/router.test.tsx` (7) proves the `/design-system` + `/skeleton` gate: with `enableQaRoutes` (default `import.meta.env.DEV`) **true** both routes register and render their pages; **false** (production) both fall through to the `*` → `/` redirect with the QA components never rendered, the landing page exposes **no** links to them, and unknown routes still redirect to `/` without matching a QA route. The pages are `React.lazy`, so a production build code-splits them **out of the main bundle** (`DesignSystemPage-*.js`, `SkeletonPage-*.js`). Real-browser evidence (git-ignored `frontend/artifacts/screenshots/repository-recovery-staging-gate/manifest.json`): dev renders both; the production preview redirects both to `/` (desktop-LTR + mobile-RTL), `/` and `/catalog` render, no console errors. This brings frontend unit tests to **413** (56 files). See [`docs/release-readiness/REPOSITORY_HISTORY_RECOVERY_AND_HYGIENE.md`](release-readiness/REPOSITORY_HISTORY_RECOVERY_AND_HYGIENE.md).
