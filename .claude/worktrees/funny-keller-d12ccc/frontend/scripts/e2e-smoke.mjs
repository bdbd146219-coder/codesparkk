...
const checks = [
  ...
  },
  {
    ...
    async run(page) {
      ...
      for (const id of ['#reg-displayName', '#reg-email', '#reg-password', '#reg-confirmPassword']) {
  ...
  },
  {
    ...
    async run(page) {
      ...
      assert.ok(
        ...
        'authed parent must not be bounced to login',
      );
    },
  },
  {
  ...
  },
  {
    ...
    async run(page) {
      ...
      await page.locator('#login-email').waitFor({ state: 'visible', timeout: 10000 });
      const anchors = await page.locator('a[href^="http"]').evaluateAll((els) =>
