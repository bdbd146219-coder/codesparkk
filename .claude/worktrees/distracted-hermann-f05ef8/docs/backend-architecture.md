...
- Migrations (apply in order on SQL Server): `InitialIdentityAuth`, `InitialCourseDomain`, `AddCategoryRowVersion`, `AddLearningPathRowVersion`.

#### Pending migrations / deployment note

`AddCategoryRowVersion` and `AddLearningPathRowVersion` add a `rowversion` concurrency column to `Categories` / `LearningPaths`. They are additive (no backfill) and low-risk. Apply with:
...
Integration tests use SQLite via `EnsureCreated()` and therefore do **not** run migrations.

## Catalog & admin catalog (Phase C backend)

The catalog backend follows the shipped service pattern — **controller → Application service interface → Infrastructure EF implementation → `IExceptionHandler` → ProblemDetails** (no MediatR).

- **Public vs admin separation.** Public read (`/api/v1/catalog/*`, anonymous, `ICatalogService`) returns only published/active content with locale-resolved text. Admin (`/api/v1/admin/*`, `IAdminCourseService` / `IAdminCategoryService` / `IAdminLearningPathService`) exposes every non-deleted state with **raw bilingual** fields.
- **Authorization.** Named policies in `CoursePolicies.AddCoursePolicies` — `Courses.{ViewDrafts,Manage,Publish,Archive}`, `Categories.Manage`, `LearningPaths.{Manage,Publish,Archive}` — all map to Admin/SuperAdmin in V1 via `RequireRole` (inspects all role claims; do **not** use the single-valued `ICurrentUser.Role` for authz).
- **Optimistic concurrency.** Course, Category, and LearningPath carry a `byte[] RowVersion`. SQL Server uses a native `rowversion`; on other providers `AppDbContext.OnModelCreating` demotes it to a plain column (no concurrency token). The service therefore enforces concurrency with an **explicit base64 comparison** before mutating (provider-independent → 409 is testable on SQLite), and EF's native rowversion is the SQL-Server backstop. Clients echo `rowVersion` on every write; stale → 409 with a `currentRowVersion` ProblemDetails extension; missing/garbled → 400.
- **Aggregate-child writes.** New children (course modules/instructors/outcomes, learning-path items) added to an already-tracked aggregate are persisted with `db.Add(child)` so EF tracks them as `Added`, not `Modified` (the latter produces a spurious `DbUpdateConcurrencyException`).
- **Publish readiness** is domain-coded (`PublishRequirement` / `LearningPathRequirement`) and mapped to `{ code, messageKey, satisfied, message }` by an Application mapper; failures return **422** with the checklist in a `readiness` extension. Learning-path publish additionally requires (service-level) at least one item referencing a currently-published course.
- **Audit.** Every catalog mutation writes an append-only `AuditEntry` via `IAuditWriter` (fresh scope, non-blocking, never rolls back the operation) with a small, safe context (`{ courseId/categoryId/learningPathId, slug, fromState, toState, … }`) — never bilingual blobs. Event names live in `CourseAuditEventTypes`.
