...
describe('InterestDialog', () => {
  ...
  it('validates required fields before calling the API', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/enter the parent or guardian name/i)).toBeTruthy());
    expect(submitMock).not.toHaveBeenCalled();
  });
  ...
  it('shows safe error copy on API failure and never leaks the raw body', async () => {
    submitMock.mockRejectedValue(
      new ApiError(500, 'https://codesparkkids.dev/errors/internal', 'errors.server', 'STACKTRACE at Db.Save()'),
    );
    renderDialog();
