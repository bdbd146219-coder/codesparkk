...
export function LearningPathFilters({ fc }: { fc: LearningPathFilterController }) {
  const { t } = useTranslation();
  const allLabel = t('staff.catalog.learningPaths.list.allOption');
  ...
  return (
    <section
      aria-label={t('staff.catalog.learningPaths.list.filtersLabel')}
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div className="w-full sm:max-w-md">
          <Label htmlFor="path-search" className="text-xs font-medium text-muted-foreground">
            {t('staff.catalog.learningPaths.list.searchLabel')}
          </Label>
          <div className="relative mt-1.5">
            ...
            <Input
              ...
              placeholder={t('staff.catalog.learningPaths.list.searchPlaceholder')}
              className="ps-9"
            />
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <FilterSelect
          id="path-filter-status"
          label={t('staff.catalog.learningPaths.list.filters.status')}
        ...
        <FilterSelect
          id="path-filter-age-band"
          label={t('staff.catalog.learningPaths.list.filters.ageBand')}
        ...
        <FilterSelect
          id="path-filter-listed"
          label={t('staff.catalog.learningPaths.list.filters.listed')}
        ...
        <FilterSelect
          id="path-filter-sort"
          label={t('staff.catalog.learningPaths.list.filters.sort')}
          value={fc.values.sort ?? ''}
          onChange={(v) => fc.setValues({ sort: v || undefined })}
        >
          <option value="">{t('staff.catalog.learningPaths.list.sort.updated')}</option>
          {SORT_VALUES.map((v) => (
            <option key={v} value={v}>
              {t(`staff.catalog.learningPaths.list.sort.${v}`)}
            </option>
          ))}
        </FilterSelect>
      </div>
