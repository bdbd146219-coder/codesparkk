    ...
    IValidator<AssignInstructorRequest> assignInstructorValidator) : IAdminCourseService
{
    ...
    public async Task<AdminCourseDetailDto> UpdateAsync(Guid id, UpdateCourseRequest request, RequestContext ctx, CancellationToken ct)
    {
        ...
        try
        {
            ...
            {
                // Replacing the owned outcomes collection wholesale trips EF Core's
                // orphan detection on SQL Server's native rowversion: a replaced child
                // is mis-tracked as Modified, so its UPDATE matches 0 rows and
                // SaveChanges throws a spurious DbUpdateConcurrencyException even when
                // the course rowVersion is current (C4J). Capturing the old rows and
                // marking them deleted + the new rows added gives EF an unambiguous
                // DELETE+INSERT plan — the same explicit-tracking approach the module
                // path uses (db.Add). Invisible on SQLite, which demotes the rowversion
                // token to a no-op concurrency check (see AppDbContext).
                var replacedOutcomes = course.Outcomes.ToList();
                course.UpdateOutcomes(request.Outcomes.Select(o => LocalizedText.Create(o.TextEn, o.TextAr)), now);
                db.RemoveRange(replacedOutcomes);
                db.AddRange(course.Outcomes);
            }
        ...
        // Concurrency is enforced by the explicit rowVersion comparison above
        // (EnsureRowVersionMatches) plus EF's loaded original value driving the
        // native rowversion WHERE clause on SQL Server. We deliberately do NOT
        // override RowVersion.OriginalValue here: combined with the aggregate-child
        // INSERT/DELETE of the outcomes rewrite it produced spurious concurrency
        // conflicts. This mirrors SaveAndReturnAsync (the module/instructor path).
        try
        {
            await db.SaveChangesAsync(ct);
        }
