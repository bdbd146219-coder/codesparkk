...
async function call<T>(
  ...
  // JSON by default — but never for FormData, whose multipart boundary the
  // browser must set itself (setting Content-Type here would corrupt it).
...
export const api = {
  ...
  postForm<T>(path: string, form: FormData, options?: RequestOptions): Promise<T> {
    return call<T>(path, { method: 'POST', body: form }, options);
  },
};
