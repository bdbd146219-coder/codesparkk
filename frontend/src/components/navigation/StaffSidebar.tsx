...
import { filterGroups, filterItems, staffNav } from '@/lib/navigation';
 ...
 * is rendered through `MobileNavSheet` (see `StaffTopBar`). Items are filtered
 * by the current role; an anonymous preview (no role) shows everything.
 */
export function StaffSidebar() {
  const { t } = useTranslation();
  const role = useCurrentRole();
  const primary = filterGroups(staffNav.primary, role);
  const utility = filterItems(staffNav.utility ?? [], role);
